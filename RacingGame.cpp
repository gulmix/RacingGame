	...
	virtual bool OnUserUpdate(float fElapsedTime) {
		...
		for (int i = 0; i < ScreenHeight() / 2; i++)
		{
			for(int j = 0; j < ScreenWidth(); j++){
				...
				float fPerspective = (float)i / (ScreenHeight() / 2.0f);
				...
				if (j >= 0 && j < nLeftGrass)
					Draw(j, nRow, PIXEL_SOLID, nGrassColour);
				if (j >= nLeftGrass && j < nLeftClip)
					Draw(j, nRow, PIXEL_SOLID, nClipColour);
				if (j >= nLeftClip && j < nRightClip)
					Draw(j, nRow, PIXEL_SOLID, nRoadColour);
				if (j >= nRightClip && j < nRightGrass)
					Draw(j, nRow, PIXEL_SOLID, nClipColour);
				if (j >= nRightGrass && j < ScreenWidth())
					Draw(j, nRow, PIXEL_SOLID, nGrassColour);
			}
		}
